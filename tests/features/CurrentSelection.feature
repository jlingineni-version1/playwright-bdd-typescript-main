Feature: Current Selection
  ...
  Scenario Outline: Verify current selection results displayed accordingly when Score for different local authorities are selected
    ...
    Then current selection should display expected results <cs_latitude>,<cs_longitude>,"<expectedLocation>","<squareID>"
    Examples:
      | location      | expectedScore | x   | y   | squareID      | cs_latitude | cs_longitude | expectedLocation       |
      | Hounslow West | 86            | 476 | 275 | 512250_176050 | 51.472289   | -0.385232    | HounslowGreater London |
  ...
  Scenario: Verify explore the score link displays expected results
    When I search for location "<location>" and select coordinates <x>,<y> on the map & select tile on the map
    Then I click on Explore the score link should display expected results <cs_latitude>,<cs_longitude>,"<expectedLocation>","<squareID>"
    Examples:
      | location      | expectedScore | x   | y   | squareID      | cs_latitude | cs_longitude | expectedLocation |
      | Hounslow West | 86            | 476 | 275 | 512250_176050 | 51.472289   | -0.385232    | Hounslow         |
